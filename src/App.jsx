...
function App() {
  ...
  // 🌟 नए State: सारे रेस्टोरेंट्स की लिस्ट और जो रेस्टोरेंट सेलेक्ट किया गया है
  const [restaurantsList, setRestaurantsList] = useState([]);
  const [activeRestId, setActiveRestId] = useState("cd92a6d1-2335-4f5e-9007-ffda681045a1"); // Default Mahiku
  const [activeRestName, setActiveRestName] = useState("Mahiku Cafe");

  useEffect(() => {
    fetchRestaurantsList(); // 👈 ऐप खुलते ही सारे रेस्टोरेंट मंगाओ
    if(activeRestId) fetchMenu(activeRestId);
    fetchOrders();
    const interval = setInterval(() => { if (isAdminLoggedIn) fetchOrders(); }, 15000);
    return () => clearInterval(interval);
  }, [isAdminLoggedIn, activeRestId]);

  // 🏪 डेटाबेस से रेस्टोरेंट खींच कर लाना
  const fetchRestaurantsList = () => {
    fetch('https://eater-backend.onrender.com/api/restaurants')
      .then(res => res.json())
      .then(data => setRestaurantsList(data.data || []))
      .catch(err => console.log("Failed to fetch restaurants"));
  };
  ...
  const updateOrderStatus = async (orderId) => {
    await fetch(`https://eater-backend.onrender.com/api/orders/update-status`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ orderId, status: 'DELIVERED' }) });
    fetchOrders();
  };

  const handleAddItem = async (e) => {
    e.preventDefault();
    const res = await fetch('https://eater-backend.onrender.com/api/menu', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ...newItem, restaurantId: activeRestId, price: Number(newItem.price), category: "Fast Food", isVeg: true }) });
    if (res.ok) { alert("✅ आइटम जुड़ गया!"); setNewItem({ name: '', price: '', description: '' }); fetchMenu(activeRestId); }
  };

  const printBill = (order, index) => {
    const printWindow = window.open('', '_blank');
    printWindow.document.write(`<html><head><style>body{font-family:'Courier New',monospace;text-align:center;padding:20px}.bill{border:1px dashed #000;padding:15px;width:260px;margin:auto}.line{border-top:1px dashed #000;margin:10px 0}</style></head><body><div class="bill"><h2>${activeRestName} 🏪</h2><p>Order #${orders.length - index} | ${new Date(order.createdAt).toLocaleDateString()}</p><div class="line"></div><p style="text-align:left"><strong>Cust:</strong> ${order.user?.name || 'Guest'}<br><strong>Phone:</strong> ${order.user?.phone || 'N/A'}</p><div class="line"></div><p style="text-align:left">Fast Food / Pizza</p><h3 style="margin:10px 0">Total: ₹${order.totalAmount}</h3><div class="line"></div><p>धन्यवाद! 🙏</p></div><script>setTimeout(()=>{window.print();window.close();},500);</script></body></html>`);
  ...
  const registerRestaurant = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        fetchRestaurantsList(); // नया रेस्टोरेंट जुड़ते ही लिस्ट रिफ्रेश करो
        setView('home');
      }
    } catch (err) { alert("🌐 नेटवर्क एरर!"); }
  };
  ...
  return (
    <div className="app-container">
      ...
      {/* 🚀 होमपेज: अब यहाँ सारे डायनामिक रेस्टोरेंट दिखेंगे! */}
      {view === 'home' && (
        <main className="home-content">
          <div className="hero-banner"><h2>Delicious Food, Delivered Fast 🚀</h2><p>Order from top restaurants near you!</p></div>
          <div className="restaurant-grid">
            {restaurantsList.length === 0 ? <p style={{textAlign: 'center', width: '100%'}}>Loading restaurants...</p> : restaurantsList.map(rest => (
              <div key={rest.id} className="rest-card" onClick={() => { 
                ...
                <h3>{rest.name}</h3>
                <p style={{color: '#64748b', fontSize: '14px'}}>{rest.location || "Kanpur / Unnao"}</p>
              </div>
            ))}
          </div>
        </main>
      ...
      {/* बाकि का UI सेम रहेगा (Menu, Admin, Register) */}
      {view === 'menu' && (
        <div className="main-container">
          ...
          <div className="customer-layout">
            ...
            <div className="cart-sidebar">
              ...
              {cart.length > 0 && (<><hr style={{borderColor: '#f1f5f9'}} /><h3 style={{display: 'flex', justifyContent: 'space-between'}}>Total: <span>₹{cart.reduce((a, b) => a + Number(b.price), 0)}</span></h3><button className="checkout-btn" onClick={placeCartOrder}>Place Order Now</button></>)}
            </div>
          </div>
        </div>
      )}
        ...
        <div className="main-container">
          ...
          <div className="login-form-container" style={{ maxWidth: '500px', margin: 'auto', background: 'white', padding: '30px', borderRadius: '16px', boxShadow: '0 10px 25px rgba(0,0,0,0.05)' }}>
            ...
            <form onSubmit={registerRestaurant} style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
              ...
              <input type="text" placeholder="FSSAI License (Optional)" value={vendorForm.fssai} onChange={e => setVendorForm({...vendorForm, fssai: e.target.value})} style={{ padding: '12px', borderRadius: '8px', border: '1px solid #cbd5e1' }} />
      ...
      {view === 'admin' && (
        <div className="main-container">
          {!isAdminLoggedIn ? (
            ...
            <>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}><h2 style={{ margin: 0 }}>{activeRestName} Dashboard 📈</h2><button onClick={() => {setIsAdminLoggedIn(false); setPassword('');}} className="back-btn" style={{ margin: 0 }}>Logout</button></div>
              <div className="summary-cards" style={{display:'flex', gap:'15px', marginBottom:'30px'}}>
                <div className="card" style={{background:'#f0fdf4', padding:'20px', borderRadius:'12px', flex:1, border:'1px solid #bbf7d0'}}><p style={{margin:0, color:'#16a34a', fontWeight:'600'}}>Today's Sales</p><h2 style={{margin:'5px 0 0 0', fontSize:'28px'}}>₹{todaySales}</h2></div>
              </div>
              <div style={{background: 'white', padding: '20px', borderRadius: '12px', marginBottom: '30px', boxShadow: '0 4px 6px rgba(0,0,0,0.02)'}}>
                <h3 style={{marginTop: 0}}>Add New Dish to {activeRestName} 🍲</h3>
                <form onSubmit={handleAddItem} style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
                  ...
                  <button type="submit" className="add-btn" style={{width: 'auto'}}>Save</button>
                </form>
              </div>
              ...
              <div className="admin-menu-list">
                {orders.slice().reverse().map((order, index) => (
                  <div key={order.id} className="admin-menu-item" style={{background: order.status==='DELIVERED'?'#f8fafc':'#f0fdf4', display:'flex', justifyContent:'space-between', padding:'15px', marginBottom:'12px', border: order.status==='DELIVERED'?'1px solid #e2e8f0':'1px solid #bbf7d0'}}>
                    <div><div style={{fontSize: '18px'}}>🛒 <strong>Order #{orders.length - index}</strong> - <span style={{color: '#ea580c', fontWeight: 'bold'}}>₹{order.totalAmount}</span></div><div style={{fontSize: '14px', color: '#64748b', marginTop: '5px'}}>👤 {order.user?.name || 'Guest'} | 📱 {order.user?.phone || 'N/A'}</div></div>
                    <div style={{display:'flex', gap:'10px', alignItems: 'center'}}><button onClick={() => printBill(order, index)} style={{background:'#3b82f6', color:'white', border:'none', padding:'8px 15px', borderRadius:'6px', cursor: 'pointer'}}>Print 🖨️</button>{order.status === 'PENDING' ? (<button onClick={() => updateOrderStatus(order.id)} style={{background:'#16a34a', color:'white', border:'none', padding:'8px 15px', borderRadius:'6px', cursor: 'pointer'}}>Deliver ✅</button>) : (<span style={{color: '#94a3b8', fontWeight: 'bold', padding: '0 10px'}}>DELIVERED</span>)}</div>
                  </div>
                ))}
              </div>
            </>
